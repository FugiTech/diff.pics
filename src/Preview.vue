<template>
  <div class="preview-main">
    <h1>{{ title }}</h1>
    <div class="subtitle"><a href="#download" @click.prevent="download">{{ $t('Download') }}</a> - {{ $t('Views', {views: 0}) }}</div>
    <div class="selector">
    ...
    <div class="subselector" v-show="images[selected].length > 2">
      <span v-for="(img, idx) in images[selected]" @click.prevent="view = idx">{{ label(idx+1) }}: {{ name(selected, idx) }}</span>
    </div>
