...
{{ range $idx, $row := .Images }}
    <a href="/{{ $.Key }}/{{ incr $idx }}"><img src="/thumbs/{{ (index $row 0).Thumb }}" onerror="this.onerror=null;this.src='https://download.diff.pics/thumbs/{{ (index $row 0).Thumb }}'"></a>
{{ end }}
  </div>
{{ end }}
...
{{ range $idx, $img := index .Images .Selected }}
  <div class="comparison comp-{{ incr $idx }}">
    <h2>{{ $img.Name }}</h2>
    <img src="/images/{{ $img.Path }}" onerror="this.onerror=null;this.src='https://download.diff.pics/images/{{ $img.Path }}'" />
  </div>
{{ end }}
{{ if le (len (index .Images .Selected)) 2 }}
</div>
